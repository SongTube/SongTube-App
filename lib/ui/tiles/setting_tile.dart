    ...
    return ListTile(
      ...
      title: Text(title,
          style: subtitleTextStyle(context, bold: true)
              .copyWith(color: enabled ? null : Colors.grey.withOpacity(0.6))),
    ...
    return ListTile(
      ...
      subtitle: Text(subtitle,
          style: tinyTextStyle(context, opacity: 0.7)
              .copyWith(color: enabled ? null : Colors.grey.withOpacity(0.6))),
      trailing:
          CircularCheckbox(value: value, onChange: enabled ? onChange : null),
    );
  }
}
      ...
      children: [
        ListTile(
          leading: SizedBox(
            height: double.infinity,
            child:
                Icon(widget.leadingIcon, color: Theme.of(context).primaryColor),
          ),
          title:
              Text(widget.title, style: subtitleTextStyle(context, bold: true)),
  ...
  Widget _slider() {
    return SliderTheme(
        ...
        child: Row(
          children: [
            Text(
                (dragValue ?? widget.value).round().toString() +
                    widget.valueTrailingString,
                style: tinyTextStyle(context, opacity: 0.7)),
            ...
            const SizedBox(width: 4),
            Text(widget.max.round().toString() + widget.valueTrailingString,
                style: tinyTextStyle(context, opacity: 0.7)),
          ],
        ));
  }
      ...
      children: [
        Expanded(
          child: ListTile(
            ...
            subtitle:
                Text(subtitle, style: tinyTextStyle(context, opacity: 0.7)),
          ),
        ),
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16),

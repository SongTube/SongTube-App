...
class FancyScaffoldState extends State<FancyScaffold> with TickerProviderStateMixin {
            ...
            return Stack(
              children: [
                  ...
                  body: widget.floatingWidgetConfig.enableGlobalPlayer ? WillPopScope(
                    onWillPop: () async {
                      if (widget.floatingWidgetController!.navigatorKey.currentState?.canPop() ?? false) {
                        widget.floatingWidgetController!.navigatorKey.currentState!.pop();
                        return false;
                      }
                      return true;
                    },
                ...
                AnimatedSwitcher(
                  ...
                  child: widget.musicFloatingWidget != null || widget.videoFloatingWidget != null
                    ? Stack(
                        ...
                        children: [
                          ...
                          Builder(
                            builder: (context) {
                              return Padding(
                                padding: EdgeInsets.only(bottom: (78 * navigationBarAnimationController.value) + (systemBottomPadding * (1 - floatingWidgetAnimationController.value))),
            ...
            child: NotificationListener<ScrollUpdateNotification>(
              onNotification: (ScrollUpdateNotification details) {
                ...
                if ((details.scrollDelta ?? 0) > 0.0 && details.metrics.axis == Axis.vertical) {
                  ...
                  if (details.metrics.pixels >= details.metrics.maxScrollExtent) return false;
                  navigationBarAnimationController.value += pixelsScrolled;
                }
                return false;
              },
  ...
  Widget _musicSlidingPanel() {
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        return SizedBox(
          height: ((widget.floatingWidgetConfig.maxHeight ?? MediaQuery.of(context).size.height) *
      ...
      return Container(
        ...
        child: Row(
          children: [
                      ...
                      return Dismissible(
                        ...
                        onDismissed: (direction) async {
                          if (uiProvider.currentPlayer == CurrentPlayer.video) {
                            ...
                            uiProvider.switchPlayers();
                            contentProvider.endVideoPlayer();
                            setState(() {
                              dismissKey = ValueKey(const Uuid().v4());
                            });
                            // Restore default is the other player is not active
                            if (widget.musicFloatingWidget == null) {
                              uiProvider.paletteLoaderController.switchToDefaultColors();
                            }
                          } else {
            ...
            if (widget.videoFloatingWidget != null && widget.musicFloatingWidget != null)
            AnimatedBuilder(
              animation: floatingWidgetAnimationController,
              builder: (context, snapshot) {
                return Opacity(
                  opacity: 1*(1-(floatingWidgetAnimationController.value*4).clamp(0, 1)),
                  child: Material(
                    color: Colors.transparent,
                    child: Bounce(
                      ...
                      child: Container(
                        height: 48*(1-(floatingWidgetAnimationController.value*4).clamp(0, 1)),
                        width: 48*(1-(floatingWidgetAnimationController.value*4).clamp(0, 1)),
                        margin: EdgeInsets.only(right: 12*(1-(floatingWidgetAnimationController.value*4).clamp(0, 1))),
                        decoration: BoxDecoration(
    ...
    return !widget.floatingWidgetConfig.backdropEnabled ? Container() : AnimatedBuilder(
      ...
      builder: (context, _) {
        return Container(
          ...
          color: floatingWidgetAnimationController.value == 0.0 ? null
            : Theme.of(context).cardColor.withOpacity(
                widget.floatingWidgetConfig.backdropOpacity *
                (floatingWidgetAnimationController.value*2)),
        );
      }
    );
  }
  ...
  Widget _gestureHandler({required Widget child}){
    if (widget.musicFloatingWidget != null || widget.videoFloatingWidget != null){
  ...
  void _onGestureSlide(double dy){
    ...
    if(!_scrollingEnabled){
      floatingWidgetAnimationController.value -= dy /
        ((widget.floatingWidgetConfig.maxHeight ?? MediaQuery.of(context).size.height)
         - widget.floatingWidgetConfig.minHeight);
    }
  ...
  void _onGestureEnd(Velocity v){
    ...
    double d2Snap = (3 -floatingWidgetAnimationController.value).abs(); // large value if null results in not every being the min
